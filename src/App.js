  ...
  const startListening = () => {
    if (recognition && !isListening) {
      try {
        ...
        setIsListening(false);
        alert('Ошибка запуска голосового ввода. Попробуйте ещё раз.');
      }
    } else if (!recognition) {
...
function AIAssistantPage() {
  ...
  const askAI = async (prompt) => {
    ...
    try {
      ...
      if (!apiKey || apiKey === 'your_gemini_api_key_here' || apiKey === 'your_gemini_key_here') {
        console.log('No valid API key found');
        // Показываем локальные советы вместо ошибки
        const localTips = {
        ...
        const localResponse = localTips[prompt] || `🤖 Локальные советы:\n\nК сожалению, AI ассистент не настроен.\n\nПопробуйте одну из быстрых команд выше или создайте задачи вручную.`;
